...
class NotificationService:
    ...
    def notify_user(self, subject:str, message:str)->None:
        ...
        methods = [
            ('email', lambda:EmailSender.send_email(self.user_data['email'], subject, message)),
            ('sms', lambda:SMSSender.send_sms(self.user_data['phone'], message)),
            ('telegram', lambda:TelegramSender.send_telegram_message(self.user_data['telegram_chat_id'], message))
        ]
        ...
        for method_name, send_func in methods:
            ...
            if success:
                ...
                print(f'{method_name.capitalize()} не удался. Переходим к следующему...')
        else:
